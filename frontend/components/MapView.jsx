  ...
  return (
    <div ref={containerRef} className="absolute left-4 right-4 top-4 z-[500] max-w-xl lg:right-auto lg:w-[28rem]">
      <form
        ...
        className="flex overflow-hidden rounded-2xl bg-white shadow-xl shadow-slate-900/12 ring-1 ring-slate-200"
      >
        <input
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          className="min-w-0 flex-1 bg-white px-4 py-3 text-sm font-medium text-slate-950 outline-none placeholder:text-slate-400"
        ...
        <button
          ...
          className="shrink-0 bg-slate-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-wait disabled:bg-slate-400"
        >
          {isSearching ? '...' : 'Suchen'}
        </button>
      </form>
      {(results.length > 0 || error) && (
        <div className="mt-2 overflow-hidden rounded-2xl bg-white shadow-xl shadow-slate-900/12 ring-1 ring-slate-200">
          ...
          {results.map((result) => (
            <button
              ...
              className="block w-full border-b border-slate-100 px-4 py-3 text-left text-sm text-slate-700 transition last:border-b-0 hover:bg-emerald-50 hover:text-emerald-900"
            >
...
export default function MapView({
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-slate-100">
          ...
          >
            <Popup>
              <div className="max-w-56 space-y-2 text-sm text-slate-800">
                ...
                <p className="leading-5">{place.description}</p>
      ...
      <div className="pointer-events-none absolute left-4 top-[5.25rem] z-[400] rounded-full bg-white/95 px-4 py-2 text-xs font-semibold text-slate-700 shadow-lg shadow-slate-900/10 ring-1 ring-slate-200 lg:top-20">
        Klicke auf die Karte, um einen Ort hinzuzufuegen
      </div>
      <div className="pointer-events-none absolute bottom-4 left-4 z-[400] rounded-full bg-slate-950/90 px-3 py-2 text-xs font-semibold text-white shadow-lg shadow-slate-900/20">
